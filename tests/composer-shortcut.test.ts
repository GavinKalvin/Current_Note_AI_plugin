...
describe("shouldSubmitComposer", () => {
  it.each([
    { name: "Shift+Enter", event: { key: "Enter", shiftKey: true } },
    { name: "Shift+numpad Enter", event: { key: "Unidentified", code: "NumpadEnter", shiftKey: true } },
    { name: "legacy Electron Shift+Enter", event: { key: "Unidentified", keyCode: 13, shiftKey: true } },
  ...
  ])("keeps $name as a newline", ({ event }) => {
    expect(shouldSubmitComposer(event)).toBe(false);
  });

  it("does not submit while an input method is composing text", () => {
    expect(shouldSubmitComposer({ key: "Enter", shiftKey: true, isComposing: true })).toBe(false);
    expect(shouldSubmitComposer({ key: "Enter", shiftKey: true, keyCode: 229 })).toBe(false);
  });
